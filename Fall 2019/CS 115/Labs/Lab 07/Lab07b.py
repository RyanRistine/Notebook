...
def draw_sudan_flag(flag_width):
    ...
    for i in range(3):
        ...
        stripe_bottom_right = Point(flag_width, (i +1) * stripe_height)
...
def draw_france_flag(flag_width):
    ...
    for i in range(3):
        ...
        stripe_bottom_right = Point((i + 1) * stripe_width, flag_height)
        draw_stripe(window, stripe_top_left, stripe_bottom_right, stripe_colors[i])
    # Wait for keyboard, to close the window
    input('Press ENTER to close the flag window.')
    window.close()
...
def draw_bangladesh_flag(flag_width):
    ...
    flag_center = Point(flag_width * 9/20, flag_height * 9/20)
...
# --- Our main function ---
def main():
    flag_width = int(input("Window width: "))
    if(flag_width < 100 or flag_width > 1000):
        sys.exit('Error: Window size should be between 100 and 1000.')
    else:
        flag = input("What flag(s) would you like to draw?\n- Japan\n- Bangladesh\n- France\n- Russia\n- Sudan\nName Your Country: ")
        flag = flag.lower()
        flags = flag.split()
        for c_flag in flags:
            if(c_flag == 'japan'):
                draw_japan_flag(flag_width)
            elif(c_flag == 'bangladesh'):
                draw_bangladesh_flag(flag_width)
            elif(c_flag == 'france'):
                draw_france_flag(flag_width)
            elif(c_flag == 'russia'):
                draw_russia_flag(flag_width)
            elif(c_flag == 'sudan'):
                draw_sudan_flag(flag_width)
            else:
                print("Error: ", c_flag, " is not a valid country", sep ="")
